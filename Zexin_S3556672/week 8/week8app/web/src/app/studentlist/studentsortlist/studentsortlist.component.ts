...
export class StudentsortlistComponent {
  ...
  displayedColumns: string[] = ['StudentId', 'StudentName', 'ClassroomId','Action'];
  tableFilter() {
  ...
  openDialogDelete(student:Student){
    this.matdialog.open(StudentDialogDeleteComponent, {width:'30%',data: {
      StudentId : student.StudentId,
      StudentName : student.StudentName,
      ClassroomName : this.getClassroomById(student.ClassroomId)?.ClassroomName,
    }});
    this.matdialog.afterAllClosed.subscribe((res)=>this.refresh())
  }
  ...
  constructor(
    ...
    private classroomserviceService:ClassroomserviceService
  ) {
    this.refresh()
  }
public refresh(){
  ...
  getClassroomById(classroomId: number): Classroom | undefined {
    return this.classroom.find(c => c.ClassroomId === classroomId);
  }
}
